...
start_state['attitude'] = [0, 0, 0]
start_state['velocity'] = [0, 0, 0]
start_state['angularv'] = [0, 0, 0]
start_state['time_step'] = 0
...
#search_space = sigmoidspace(0, max_rotor_speed, motor_speed_sampling_steps)
...
while not possible_drone_states.empty():
	...
	if current_processing_state['time_step'] < total_time_steps:
			...
			for w2 in search_space:
				for w3 in search_space:
					for w4 in search_space:
						...
						new_drone_state = copy.deepcopy(current_processing_state)
						...
						omega_x = new_drone_state['angularv'][0]
						omega_y = new_drone_state['angularv'][1]
						omega_z = new_drone_state['angularv'][2]
						...
						new_drone_state['angularv'][0] += derivative_omega_array[0][0]
						new_drone_state['angularv'][1] += derivative_omega_array[1][0]
						new_drone_state['angularv'][2] += derivative_omega_array[2][0]
						...
						r = new_drone_state['attitude'][0]
						p = new_drone_state['attitude'][1]
						y = new_drone_state['attitude'][2]
						...
						new_drone_state['attitude'][0] += rpy_derivative[0][0]
						new_drone_state['attitude'][1] += rpy_derivative[1][0]
						new_drone_state['attitude'][2] += rpy_derivative[2][0]
						...
						new_drone_state['velocity'][0] += acceleration_array[0][0]
						new_drone_state['velocity'][1] += acceleration_array[1][0]
						new_drone_state['velocity'][2] += acceleration_array[2][0]

						new_drone_state['position'][0] += new_drone_state['velocity'][0]
						new_drone_state['position'][1] += new_drone_state['velocity'][1]
						new_drone_state['position'][2] += new_drone_state['velocity'][2]

						new_drone_state['time_step'] += 1

						# Check to make sure the drone has not hit the floor
						if new_drone_state['position'][2] > 0:
	...
	plotting_drone_state.put(copy.deepcopy(current_processing_state))
...
while not plotting_drone_state.empty():
	...
	x_pos.append(drone_state['position'][0])
	y_pos.append(drone_state['position'][1])
	z_pos.append(drone_state['position'][2])
