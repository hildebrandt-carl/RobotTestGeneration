...
//  initialPose.position.y = 2.0f;
  ...
  gpsPub_ = node_.advertise<geometry_msgs::Pose>("/uav/sensors/gps", 96);
...
void Uav_Dynamics::publishState(void){
  ...
  geometry_msgs::Pose GPSReading ;

  GPSReading.position.x = position_[0];
  GPSReading.position.y = position_[1];
  GPSReading.position.z = position_[2];

  GPSReading.orientation.x = attitude_[0];
  GPSReading.orientation.y = attitude_[1];
  GPSReading.orientation.z = attitude_[2];
  GPSReading.orientation.w = attitude_[3];
