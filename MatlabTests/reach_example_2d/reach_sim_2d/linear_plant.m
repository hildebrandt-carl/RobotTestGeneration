...
syms x y z xd yd zd qW qX qY qZ p q r
syms force M1 M2 M3
...
% Acceleration
accel = 1 / params.mass * (wRb * [0; 0; F] - [0; 0; params.mass * params.grav] + ext_force);
...
omega = [p;q;r];
pqrdot   = params.invI * (M - cross(omega, params.I*omega) + ext_moment);
...
X = [x y z xd yd zd qW qX qY qZ p q r];
U = [force M1 M2 M3];

A = matlabFunction(jacobian(f,X), 'file', 'plant_A');
B = matlabFunction(jacobian(f,U), 'file', 'plant_B');
