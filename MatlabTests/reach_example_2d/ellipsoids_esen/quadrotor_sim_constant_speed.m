...
U{1} = [1 1 1]' + 0.1 * ell_unitball(3); 
U_applied{1} = [1 1 1]' + [0.1*rand(); 0.1*rand(); 0.1*rand()];
%U = ell_unitball(3);
...
subplot(2, 1, 1);
plot_ea(ps, 'g'); hold on; grid on; xlim([0 12]); ylim([-5 15]); zlim([-20 20]); % plot the whole reach tube 
 subplot(2, 1, 2);
...
while (norm(error{i-1}) > 0.1)
    ...
    if (k == step_number*(i-1))
        ...
        U{i} = [1 1 1]' + 0.1 * ell_unitball(3); 
        U_applied{i} = [1 1 1]' + [0.1*rand(); 0.1*rand(); 0.1*rand()];
